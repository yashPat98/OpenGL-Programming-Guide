// ------------------------
// Name :        Yash Patel
// Assignment :  Icosahedron Assignment
// Date :        15-11-2020
// ------------------------
...
void RenderIcosaheron(void)
{
    ...
    glBegin(GL_TRIANGLES);
        for(i = 0; i < 20; i++)
        {
            ...
            for(j = 0; j < 3; j++)
            {
                vec1[j] = vdata[tindices[i][0]][j] - vdata[tindices[i][1]][j];
                vec2[j] = vdata[tindices[i][1]][j] - vdata[tindices[i][2]][j];
            }
            CrossProduct(vec1, vec2, norm);
